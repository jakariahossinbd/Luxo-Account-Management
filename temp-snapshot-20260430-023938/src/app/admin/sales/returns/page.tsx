    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">{t('admin.salesPayment.returns')}</h1>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-sm font-medium text-slate-600">{t('admin.salesPayment.returns')}</p>
            <p className="mt-2 text-3xl font-bold text-slate-900">{returns.length}</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-sm font-medium text-slate-600">{t('admin.salesPayment.totalAmount')}</p>
            <p className="mt-2 text-3xl font-bold text-slate-900">{formatTaka(returns.reduce((sum, sale) => sum + sale.total, 0))}</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="p-6">
            <h2 className="text-lg font-semibold text-slate-900">{t('admin.salesPayment.returns')}</h2>
          </div>
                ...
                <tbody>
                  {returns.map((sale) => (
                    <tr key={sale.id} className="border-t border-slate-200 hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 text-sm text-slate-600">{sale.status}</td>

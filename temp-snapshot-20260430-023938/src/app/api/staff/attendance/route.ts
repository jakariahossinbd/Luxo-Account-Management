...
import { buildMonthlyAttendanceDataset, formatDateKey } from '@/lib/attendance/monthlyAttendance';
...
export async function GET(request: Request) {
  try {
    ...
    const { rows: monthRows, stats } = buildMonthlyAttendanceDataset({
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        last7Days,
        monthRecords: monthRows.map((item) => ({
          id: item.id,
          date: item.date,
          status: item.status,

...
async function createCompressedPreviewDataUrl(file: File): Promise<string> {
  ...
  return compressed.length > 0 ? compressed : dataUrl;
}
...
export default function SellerDashboardHome() {
  ...
  const handleSalesSampleImage = async (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;

    try {
      const previewUrl = await createCompressedPreviewDataUrl(file);
      setCreateSalesForm((prev) => ({
        ...prev,
      ...
      warning('Image preview generation failed. Please try another image.');
    }
    event.target.value = '';
  };
            ...
            <section className="rounded-[18px] border border-slate-200 bg-white px-4 pb-4 pt-2.5 shadow-[0_8px_26px_rgba(15,23,42,0.06)]">
              ...
              <div className="relative mt-3">
                ...
                <div className="w-full bg-white px-0 py-0">
                  <div className="grid grid-cols-2 rounded-[10px] overflow-hidden border border-slate-200 text-center text-[12px] font-normal text-slate-500 sm:grid-cols-3 sm:text-[13px]">
                    <div className="border-b border-r border-slate-200 bg-slate-50 px-2 py-0.5 sm:border-b-0">
                      <p className="text-slate-600 font-semibold leading-tight">Total Delivery</p>
                      <p className="mt-0 text-sm font-semibold text-blue-600">৳{formatTakaAmount(currentMonthDeliverySummary)} TK</p>
                    </div>
                    <div className="border-b border-slate-200 bg-slate-50 px-2 py-0.5 sm:border-b-0">
                      <p className="text-slate-600 font-semibold leading-tight">Total Canceled</p>
                      <p className="mt-0 text-sm font-semibold text-red-500">৳{formatTakaAmount(currentMonthCanceledSummary)} TK</p>
                    </div>
                    <div className="col-span-2 bg-white px-3 py-0.5 text-center sm:col-span-1">
                      <h2 className="whitespace-nowrap text-[19px] font-semibold sm:text-[24px]">
                        ...
                        <span className="text-[20px] font-semibold text-emerald-600 sm:text-[26px]">৳{formatTakaAmount(selectedTotalSaleSummary)} TK</span>
                      </h2>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2.5">
                    {dashboardSummaryCards.map((card, index) => {
                      ...
                      return (
                        <article key={card.key} className={`relative flex h-[88px] flex-col overflow-hidden rounded-[10px] px-3 py-1 text-white shadow-none ${card.color}`}>
                          <div className="flex flex-1 items-center justify-between">
                            ...
                            <div className={`ml-2 w-full text-right leading-none opacity-95 [text-shadow:none] ${isAmountCard ? 'whitespace-nowrap text-[17px] font-semibold tracking-[-0.02em] sm:text-[18px]' : 'text-[44px] font-semibold sm:text-[46px]'}`}>
                              {card.label}
                            </div>
                          </div>
                ...
                {quickActions.map((action) => {
                  ...
                  return (
                    <button
                      ...
                      className="seller-menu-hover group relative flex flex-col items-center justify-center rounded-xl border border-slate-200 bg-gradient-to-b from-white to-slate-50 px-3.5 py-3 text-center shadow-sm transition-all duration-300 active:scale-[0.98]"
                    >
                      ...
                      <span className="mt-2.5 whitespace-nowrap text-center text-[11px] font-medium leading-none text-slate-700 bangla-font transition-colors duration-200 group-hover:text-orange-500">{t(action.labelKey)}</span>
                    </button>
                  );
                })}
              </div>

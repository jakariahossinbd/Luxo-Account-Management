...
function extractAmountFromNotes(notes: string | null) {
  ...
  try {
    ...
    const explicitTotalCandidates = [parsed.totalTaka, parsed.tt, parsed.total, parsed.amount];
    ...
    const candidates = [parsed.subtotal, parsed.subTotal, parsed.st];
...
function hasDeliveryStatus(stage: string, notes: string | null, strictStageOnly = false) {
  if (stage === 'DONE') {
    return true;
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [sellerUsers, sellerEmployees, deliveryRows, expenseAggregate] = await Promise.all([
    ...
    const leadsInflows = deliveryRows.reduce((sum, row) => {
      ...
      // Seller-board rows must be based on stage to avoid stale note metadata overcount.
      if (!hasDeliveryStatus(row.stage, row.notes, isSellerSourceRow)) {
        return sum;
      }
    ...
    // Inflows are computed from seller delivery rows only to avoid overlap with other sales sources.
    const inflows = leadsInflows;

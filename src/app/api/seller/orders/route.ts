...
function parseOrderMeta(notes: string | null): OrderMeta {
  ...
  try {
    ...
    return {
      ...
      sampleImageUrl:
        typeof parsed.sampleImageUrl === 'string' ? parsed.sampleImageUrl : typeof parsed.iu === 'string' ? parsed.iu : undefined,
    };
  } catch {
    return { source: 'seller-dashboard-orders' };
  }
...
function normalizeOrderPayload(item: unknown): SalesOrderPayload | null {
  ...
  return {
    ...
    sampleImageUrl: typeof row.sampleImageUrl === 'string' ? row.sampleImageUrl : undefined,
  };
}
    ...
    await prisma.$transaction(async (tx) => {
      for (const item of dedupedByOrderId) {
        ...
        const meta: OrderMeta = {
          ...
          sampleImageUrl: truncateText(item.sampleImageUrl, 60000),
        };

...
export default function SalesInvoicesPage() {
  ...
  const invoiceSales = useMemo(
    () => sales.filter((sale) => Boolean(sale.invoiceNumber)),
    [sales]
  );
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">{t('admin.salesPayment.invoices')}</h1>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-sm font-medium text-slate-600">{t('admin.salesPayment.invoices')}</p>
            <p className="mt-2 text-3xl font-bold text-slate-900">{invoiceSales.length}</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-3xl font-bold text-slate-900">{formatTaka(invoiceSales.reduce((sum, sale) => sum + sale.total, 0))}</p>
          </div>
        </div>

        <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="p-6">
            <h2 className="text-lg font-semibold text-slate-900">{t('admin.salesPayment.invoices')}</h2>
          </div>
                ...
                <tbody>
                  {invoiceSales.map((sale) => (
                    <tr key={sale.id} className="border-t border-slate-200 hover:bg-slate-50">
                      <td className="px-6 py-4 text-sm font-medium text-slate-900">{sale.invoiceNumber}</td>
                      <td className="px-6 py-4 text-sm text-slate-600">{sale.orderId}</td>

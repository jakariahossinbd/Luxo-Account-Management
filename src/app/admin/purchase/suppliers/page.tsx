...
export default function PurchaseSuppliersPage() {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(payload.error || 'Failed to save supplier');
      }
  ...
  const onDelete = async (id: string) => {
    const ok = confirm('Deactivate this supplier?');
    if (!ok) return;
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-600">Manage supplier directory for purchase orders.</p>
          </div>
          <button
            onClick={openCreate}
        ...
        <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
          {loading ? (
            <div className="p-8 text-center text-slate-600">Loading suppliers...</div>
          ) : suppliers.length === 0 ? (
            <div className="p-8 text-center text-slate-600">No suppliers found.</div>
          ) : (
            <>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-slate-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium uppercase text-slate-600">Name</th>
                      <th className="px-6 py-3 text-left text-xs font-medium uppercase text-slate-600">Phone</th>
                      <th className="px-6 py-3 text-left text-xs font-medium uppercase text-slate-600">Email</th>
                      <th className="px-6 py-3 text-left text-xs font-medium uppercase text-slate-600">Company</th>
                      <th className="px-6 py-3 text-left text-xs font-medium uppercase text-slate-600">Status</th>
                      <th className="px-6 py-3 text-left text-xs font-medium uppercase text-slate-600">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
          ...
          <div className="fixed inset-0 z-[95] flex items-center justify-center bg-black/45 p-4">
            <div className="w-full max-w-xl rounded-xl bg-white shadow-xl">
              <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
                <h2 className="text-lg font-semibold text-slate-900">{isEdit ? 'Edit Supplier' : 'Create Supplier'}</h2>
